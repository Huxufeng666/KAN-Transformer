...
from Efficient_SLayer import Efficient_SKAN, Efficient_SKAN_grouped, Efficient_SKAN_Batch
...
class KA_attention(nn.Module):
    ...
    def forward(self, q, k):
        ...
        f_out = torch.einsum('ij,ijk->ijk', self.coef_q, f_out_q).permute(2, 1, 0)
        f_out = torch.einsum('ij,ijk->ijk', self.coef_k, f_out_k).permute(2, 1, 0)
...
class Attention(nn.Module):
    def __init__(self,
        ...
        # self.attention = Grouped_KA_attention(out_dim = self.num_patches+1, patches = (self.num_patches+1), heads = self.num_heads, head_dim = head_dim, groups = 16)
...
class Mlp(nn.Module):
    ...
    """
    def __init__(self, in_features, hidden_features=None, num_patches = None, out_features=None, act_layer=nn.GELU, drop=0.):
        super().__init__()
        out_features = out_features or in_features
        hidden_features = hidden_features or in_features
        # self.fc1 = nn.Linear(in_features, hidden_features)
        self.fc1 = Efficient_SKAN_Batch(in_features, hidden_features, num_patches = num_patches+1)#,  groups = 8)
        # self.act = act_layer()
        # self.fc2 = nn.Linear(hidden_features, out_features)
        self.fc2 = Efficient_SKAN_Batch(hidden_features, out_features, num_patches = num_patches+1)#, groups = 8)
...
class Block(nn.Module):
    def __init__(self,
        ...
        self.mlp = Mlp(in_features=dim, hidden_features=mlp_hidden_dim, num_patches = num_patches, act_layer=act_layer, drop=drop_ratio)

    def forward(self, x):
        # print('=== block input shape: ', x.shape)
        x = x + self.drop_path(self.attn(self.norm1(x)))
        # print('=== block after attention shape: ', x.shape)
        x = x + self.drop_path(self.mlp(self.norm2(x)))
        # print('=== block after MLP shape: ', x.shape)
        return x
...
class VisionTransformer(nn.Module):
    def __init__(self, img_size=224, patch_size=16, in_c=3, num_classes=1000,
        ...
        if representation_size and not distilled:
            ...
            self.pre_logits = nn.Sequential(OrderedDict([
                ("fc", Efficient_SKAN(embed_dim, representation_size)),
                ("act", nn.Tanh())
            ]))
        else:
    ...
    def forward_features(self, x):
        ...
        # print('--- all embeddings shape: ', x.shape)
        x = self.blocks(x)
        x = self.norm(x)
        if self.dist_token is None:
...
def Full_Eit_base_patch16_224(num_classes: int = 1000):
    """
