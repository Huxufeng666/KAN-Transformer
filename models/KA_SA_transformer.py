...
class Grouped_KA_attention(nn.Module):
    ...
    def forward(self, q, k):
        ...
        group_size = int(heads*patches*dim*self.out_dim / self.groups)
        # print('-- group size: ', group_size, heads*patches*dim*self.out_dim, self.groups)
        f_out_q  = f_out_q.reshape(self.groups, group_size, self.num_f, batch)
        f_out_k  = f_out_k.reshape(self.groups, group_size, self.num_f, batch)
        ...
        f_out_q  = f_out_q.reshape(self.groups*group_size, batch).permute(1, 0)
        f_out_k  = f_out_k.reshape(self.groups*group_size, batch).permute(1, 0)
