...
class Block(nn.Module):
    ...
    def __init__(self, dim, num_heads, num_patches, mlp_ratio=4., qkv_bias=False, qk_scale=None, drop=0., attn_drop=0.,
        ...
        if  self.re_atten:
            ...
            self.attn = Attention(
                dim, num_heads=num_heads, num_patches = num_patches, qkv_bias=qkv_bias, qk_scale=qk_scale, attn_drop=attn_drop, proj_drop=drop, 
...
class PatchEmbed_CNN(nn.Module):
    ...
    def forward(self, x):
        # print('inside patch size/ new patch size: ', self.patch_size, ' ', self.new_patch_size)
        x = self.conv1(x)
        x = self.bn1(x)
        x = self.relu(x)
        ...
        x = self.bn2(x)
        x = self.relu(x)
        # print('inside after conv shape: ', x.shape)
        x = self.proj(x)
        # print('inside after proj conv shape: ', x.shape)
        x = x.flatten(2).transpose(1, 2)  # [B, C, W, H]
