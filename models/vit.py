...
class KA_attention_crossinf_2layer_V3(nn.Module): #####
    def __init__(self, out_dim, patches, heads, head_dim, reduce_ratio = 8, num_f = 8, base_fun = nn.SiLU(), device = 'cuda'):
        ...
        self.register_buffer('grid', (torch.arange(num_f).float() + 1))

        self.coef_qk_inner = nn.Parameter(torch.rand((patches*head_dim*heads*5, self.num_f), requires_grad = True))
        self.scale_base = nn.Parameter(torch.ones(1, 5*heads, 1, head_dim)).requires_grad_(True)
        self.scale_sp = nn.Parameter(torch.ones(1, 5*heads, 1, head_dim)).requires_grad_(True)
        
        self.lin_qk_inner = nn.Linear(head_dim, head_dim*5)
        self.lin_qk = nn.Linear(patches, patches)
        self.linear_final = nn.Linear(patches, patches)
        
        
        self.norm_init = nn.BatchNorm2d(heads*5)
        self.normalization_inner_division = nn.BatchNorm1d(heads*5*patches*self.head_dim, affine=False)
        self.normalization_inner = nn.BatchNorm1d(heads*5*patches*self.head_dim, affine=False)

        self.norm_qk = nn.BatchNorm2d(5*heads, affine=False)
        self.norm_final = nn.BatchNorm2d(heads, affine=False)
        
        self.tanh = nn.Tanh()
        self.std = nn.Parameter(3, requires_grad = True)
        
    def gaussian(self, x, mean, std):
        return torch.exp(-0.5 * ((x - mean) / std) ** 2) / (std * torch.sqrt(torch.tensor(2 * torch.pi)))
    
    def inner_function(self, qk, batch, heads, patches):
        qk_or = self.normalization_inner_division(self.lin_qk_inner(qk).reshape(int(2*batch), heads*5*patches*self.head_dim))
        
        qk_base = self.norm_init(F.linear(self.base_fun(qk_or.reshape(int(2*batch), heads*5, patches, self.head_dim)), self.base_weight))#*self.base_wrap)
        
        grid = torch.unsqueeze(self.grid, dim = 1)
        qk = torch.unsqueeze(qk_or, dim = 1)
        
        f_out_qqkk = torch.sin(grid*qk).permute(2, 1, 0)
        f_out_qqkk = torch.einsum('ij,ijk->ijk', self.coef_qk_inner, f_out_qqkk).permute(2, 1, 0)
        
        f_out_qqkk = self.normalization_inner(torch.sum(f_out_qqkk, dim = 1))#.reshape(int(2*batch), heads*5, patches, self.head_dim)
        
        return f_out_qqkk, qk_base#.reshape(int(2*batch), heads*5, patches, self.head_dim)
    ...
    def forward(self, q, k):
        ...
        f_out_qk, qk_base = self.inner_function(qk, batch, heads, patches)
        f_out_qk = f_out_qk.reshape(int(2*batch), heads*5, patches, self.head_dim)
        
        f_out_q = f_out_qk[0:batch, :, :, :]*self.scale_sp + qk_base[0:batch, :, :, :]*self.scale_base
        f_out_k = f_out_qk[batch:batch*2, :, :, :]*self.scale_sp + qk_base[batch:batch*2, :, :, :]*self.scale_base

        f_out_q = torch.unsqueeze(f_out_q, dim = 3)
        f_out_k = torch.unsqueeze(f_out_k, dim = 2)
        f_out = (f_out_q - f_out_k)
        
        f_out = self.tanh(self.lin_qk(torch.sum(f_out, dim = 4)))
        # print('linear out: ', f_out.shape)
        f_out = self.norm_qk(f_out).reshape(batch, heads, 5, patches, patches)
        # print('outer function input: ', f_out.shape)
        
        f_out = self.gaussian(f_out, mean = 0, std = self.std)
        
        f_out = torch.sum(f_out, dim = 2)
        
        f_out = self.norm_final(self.linear_final(f_out))
        attn = f_out.softmax(dim=-1)
        
        return attn

class KA_attention_crossinf_2layer_reinv_V2(nn.Module): #####
    def __init__(self, out_dim, patches, heads, head_dim, reduce_ratio = 8, num_f = 8, base_fun = nn.SiLU(), device = 'cuda'):
        super(KA_attention_crossinf_2layer_reinv_V2, self).__init__()
        ...
        #self.grid = (torch.arange(num_f).float() + 1).to(device)
        self.register_buffer('grid', (torch.arange(num_f).float() + 1))
    ...
    def inner_function(self, qk, batch, heads, patches):
        ...
        qk_or = self.normalization_inner_division(self.lin_qk_inner(qk).reshape(int(2*batch), heads*5*patches*self.head_dim))
        qk_base = self.norm_init(F.linear(self.base_fun(qk_or.reshape(int(2*batch), heads*5, patches, self.head_dim)), self.base_weight))
        ...
        f_out_qqkk = self.normalization_inner(torch.sum(f_out_qqkk, dim = 1))
        
        return f_out_qqkk, qk_base
    ...
    def forward(self, q, k):
        ...
        f_out = (f_out_q + f_out_k)
        f_out = self.lin_qk(torch.sum(f_out, dim = 4))
        f_out = self.norm_qk(f_out).reshape(batch, heads, 5, patches, patches)

        
        f_out = f_out*self.coef_qk_outer
        f_out = torch.sum(f_out, dim = 2)
...
class KA_attention_2layer_V2(nn.Module): #####
    def __init__(self, out_dim, patches, heads, head_dim, reduce_ratio = 8, num_f = 8, base_fun = nn.SiLU(), device = 'cuda'):
        super(KA_attention_2layer_V2, self).__init__()
        ...
        self.base_dim_weight = nn.Parameter(torch.rand((head_dim, head_dim), requires_grad = True))
        self.base_patch_weight = nn.Parameter(torch.rand((patches, patches), requires_grad = True))
        self.base_heads_weight = nn.Parameter(torch.rand((heads*5, heads*5), requires_grad = True))
        self.norm_init_qk = nn.BatchNorm1d(heads*5*patches*head_dim)

        self.base_wrap = nn.Parameter(torch.rand((heads*5, patches, 1)))
        self.coef_qk_inner = nn.Parameter(torch.rand((patches*head_dim*heads*5, self.num_f), requires_grad = True))
        self.scale_base = nn.Parameter(torch.ones(1, 5*heads, 1, head_dim)).requires_grad_(True)
        self.scale_sp = nn.Parameter(torch.ones(1, 5*heads, 1, head_dim)).requires_grad_(True)
        self.coef_qk_outer = nn.Parameter(torch.ones((1, 1, 5, 1, patches), requires_grad = True))
        
        self.lin_qk_inner = nn.Linear(head_dim, head_dim*5)
        self.lin_qk = nn.Linear(patches, patches)
        self.wrap_qk = nn.Parameter(torch.rand((heads*5, patches, 1)))
        self.linear_final = nn.Linear(patches, patches)
    ...
    def distributed_linear_inner(self, x, batch, heads, patches):
        x = x.reshape(int(2*batch), heads*5, patches, self.head_dim)
        dim_linear = F.linear(self.base_fun(x), self.base_dim_weight).reshape(int(2*batch), heads*5*patches*self.head_dim)
        patch_linear = F.linear(self.base_fun(x.reshape(int(2*batch), heads*5, self.head_dim, patches)), self.base_patch_weight).reshape(int(2*batch), heads*5*patches*self.head_dim)
        head_linear = F.linear(self.base_fun(x.reshape(int(2*batch), patches, self.head_dim, heads*5)), self.base_heads_weight).reshape(int(2*batch), heads*5*patches*self.head_dim)
        combined_linear = dim_linear + patch_linear + head_linear
        return self.norm_init_qk(combined_linear)
        
    def inner_function(self, qk, batch, heads, patches):
        qk_or = self.normalization_inner_division(self.lin_qk_inner(qk).reshape(int(2*batch), heads*5*patches*self.head_dim))
        qk_base = self.distributed_linear_inner(qk_or, batch, heads, patches)
    ...
    def forward(self, q, k):
        ...
        f_out = (f_out_q + f_out_k)
        f_out = self.lin_qk(torch.sum(f_out, dim = 4))*self.wrap_qk
        f_out = self.norm_qk(f_out).reshape(batch, heads, 5, patches, patches)
...
def _get_xps(z, len_numerator, len_denominator):
    ...
    """
    xps = [z]
    #print('numerator/denominator len: ', len_numerator, ' ', len_denominator)
    for _ in range(max(len_numerator, len_denominator) - 2):
        #print('xps: ', xps[-1].shape)
        xps.append(xps[-1] * z)
    xps.insert(0, torch.ones_like(z))  # Add x^0 = 1
    return torch.stack(xps, dim=1)


def _get_xps_sin(z, len_numerator, len_denominator):
    #xps = torch.zeros((z.shape[0], len_numerator, z.shape[1]))
    
    xps = torch.ones_like(z)
    xps = [xps.unsqueeze(dim = 1)]
    for i in range(1, len_numerator):
        aa = torch.sin(z*i).unsqueeze(dim = 1)
        xps.append(aa)
        #xps[:, i, :] = torch.sin(z*i)
    
    return torch.stack(xps, dim=1).squeeze()


class Rational_attention(nn.Module): #####
    def __init__(self, num_groups, device = 'cuda'):
        super(Rational_attention, self).__init__()
        
        self.device = device
        self.num_groups = num_groups
        
        initialization_json = "/home/user/sharjeel/DEIT/init.json"
        mode = 'identity'
        #mode = 'sin_identity'
        #mode = 'identity_7_4'
        #mode = 'sin_identity_7_4'
        with open(initialization_json) as json_file:
    ...
    def inner_function(self, qk, batch, heads, patches):
        B, L, D = qk.shape
        len_num = self.weight_numerator.size(1) # 6
        len_deno = self.weight_denominator.size(1) # 4
        #print('numerator shape: ', self.weight_numerator.shape) # [1, 6]
        #print('denominator shape: ', self.weight_denominator.shape) # [6, 4] 6 groups and 4 variables in denominator
        ...
        z = qk.view(B, L, group, D_per_group).permute(2, 0, 1, 3).contiguous()  # Shape: (group, B, L, D_per_group)
        z = z.view(group, B * L * D_per_group)  # Flatten for group-wise operation
        #print('z shape: ', z.shape) # [6, 403456]
        ...
        sin_xps = _get_xps_sin(z, len_num, len_deno)
        #print('all xps shapes: ', xps.shape, sin_xps.shape)
        ...
        #print('before numerator bmm: ', weight_numerator.unsqueeze(1).shape, ' ', xps.shape)
        ...
        expanded_dw = torch.cat([
            torch.ones(group, 1, device = self.device),  # 1 for the constant term of denominator
            self.weight_denominator,
            torch.zeros(group, max(0, len_num - len_deno - 1), device = self.device)  # Pad with zeros if numerator degree is higher
        ], dim=1)
        
        
        #print('before denominator bmm: ', expanded_dw.abs().unsqueeze(1).shape, ' ', xps.shape)
    ...
    def forward(self, x):
        ...
        x = x.reshape(batch, patches, heads*dim)
        # print('reshaped input: ', x.shape)
        
        result = self.inner_function(x, batch, heads, patches)
        # print('output shape: ', result.shape)        
        
        return result.reshape(batch, heads, patches, dim)

class SKAN(nn.Module):
    def __init__(self, patches, heads, head_dim, num_f = 8, base_fun = nn.SiLU(), device = 'cuda'):
        ...
        self.grid = nn.Parameter(torch.arange(num_f).float() + 1)#.to(device)
        #self.register_buffer('grid', (torch.arange(num_f).float() + 1))
        
        self.coef = nn.Parameter(torch.rand((patches*head_dim, self.num_f), requires_grad = True))
    ...
    def forward(self, f):
        ...
        # f = f.reshape(batch*heads*patches, dim)
        f = f.reshape(batch*heads, patches*dim)
        f = torch.unsqueeze(f, dim = 1)
        grid = torch.unsqueeze(self.grid, dim = 1)

        f_out_f = torch.sin(grid*f).permute(2, 1, 0)
        f_out_f = torch.einsum('ij,ijk->ijk', self.coef, f_out_f).permute(2, 1, 0)
        f_out_f = torch.sum(f_out_f, dim = 1)# * self.scale1
        
        return f_out_f.reshape(batch, heads, patches, dim)

from pathlib import Path

def test_function_graph(function, x_input, y_output):
    print('test input shape: ', y_output.shape)
    function_name = 'Sin_Rational_Divergence'
    '''x_data = torch.linspace(-3, 3, y_output.shape[2]*y_output.shape[3]).to('cuda:1')
    x_data = torch.unsqueeze(x_data, dim = 0)
    x_data = torch.unsqueeze(x_data, dim = 0)
    x_data = torch.unsqueeze(x_data, dim = 0)
    x_data = x_data.reshape(x_data.shape[0], 197, 64)
    x_data = x_data.repeat(y_output.shape[1], 1, 1).unsqueeze(0)'''
    ...
    #print('--- x, y data shape: ', x_data.shape, y_output.shape)
...
def check_hist(infeat, name):
    print('check hist input: ', infeat.shape)
    bins = 50
    x = range(bins)
    print('----- ', torch.min(infeat), torch.max(infeat))
    qdisp = infeat.clone()# - torch.min(q)
    print('----- ', torch.min(qdisp))
        
    for k in range(infeat.shape[1]):
      hist_before = torch.histc(qdisp[:, k, :, :].float(), bins=50, min=-5, max=5, out=None) 
      plt.bar(x, hist_before.detach().cpu().numpy())
      
      sname = name + '_' + str(k) + '.png'
      plt.savefig(os.path.join('/home/user/sharjeel/DEIT/saves/distribution_saves/', sname))
      plt.clf()


class Divergence(nn.Module):
    def __init__(self, dim, patches, heads):
        super().__init__()
        
        self.norm_init_q = nn.LayerNorm(dim)
        self.norm_init_k = nn.LayerNorm(dim)
        
        #self.linear_q = nn.Linear(dim, dim, bias = False)
        #self.linear_q = SKAN(patches, heads, dim)
        self.linear_q = Rational_attention(heads, device = 'cuda:1')
        self.norm_q = nn.LayerNorm(dim)
        
        #self.linear_k = nn.Linear(dim, dim, bias = False)
        #self.linear_k = SKAN(patches, heads, dim)
        self.linear_k = Rational_attention(heads, device = 'cuda:1')
    ...
    def forward(self, q, k):
        #print('divergence input shape: ', q.shape)
        #print('--- check q divergence before ---')
        #check_divergence(q)
        #print('input q min max: ', torch.min(q), torch.max(q))
        #print('input k min max: ', torch.min(k), torch.max(k))
        
        qb = self.norm_init_q(q)
        k = self.norm_init_k(k)

        #bins = 50
        #x = range(bins)
        #print('----- ', torch.min(q), torch.max(q))
        #qdisp = q.clone()# - torch.min(q)
        #print('----- ', torch.min(qdisp))
        

        #check_hist(q, 'before_distribution')
        
        #print('Before div Uniques: ', torch.unique(qb))
        #test_function_graph(self.linear_q, qb, q)
        q = self.norm_q(self.linear_q(qb))
        k = self.norm_k(self.linear_k(k))

        #print('--- check q divergence after ---')
        #check_divergence(q)
        
        #check_hist(q, 'after_distribution')
        
        #print('After div Uniques: ', torch.unique(q))
...
def check_divergence(x):
    ...
    heads = x.shape[1]
    print('divergence input shape: ', x.shape)
    dummy_copy = torch.clone(x)
    dummy_copy2 = torch.clone(x)
    
    dummy_copy = F.log_softmax(dummy_copy)
    dummy_copy2 = F.softmax(dummy_copy2)

    results = torch.empty(heads, heads)
    for i in range(heads):
        for j in range(heads):
            #print('-> ', F.kl_div(dummy_copy[:, i, :, :], dummy_copy2[:, j, :, :], reduction='mean').sum(-1))
            results[i, j] = F.kl_div(dummy_copy[:, i, :, :], dummy_copy2[:, j, :, :], reduction='mean').sum(-1)
    
    print('loop results shape: ', results.shape)
    print('loop results mean: ', results.mean())
...
class Attention(nn.Module):
    ...
    def __init__(
        ...
        #self.kan_attention = KA_attention_2layer_V2(out_dim = num_patches+1, patches = num_patches+1, heads = self.num_heads, head_dim = self.head_dim)#, groups = 4)
        self.test_attention = Divergence(dim = self.head_dim, patches = num_patches+1, heads = self.num_heads)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        B, N, C = x.shape
        
        #print('attention input shape: ', x.shape)
        #print('attention input min max: ', torch.min(x), torch.max(x))
        ...
        #attn = self.kan_attention(q, k)
        #print('----------------------------------------------------')
        attn = self.test_attention(q, k) * self.scale
        attn = attn.softmax(dim=-1)
        #print('attention min max: ', torch.min(attn), torch.max(attn))
...
class Block(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        #print('Block input: ', torch.min(x), torch.max(x))
        d = x.get_device()
        n = self.norm1(x)
        nn = self.attn(n)
        x = x + self.drop_path1(self.ls1(nn))
...
class Position_embed(nn.Module):
    def __init__(self, pos_embed, cls_token, no_embed_class, pos_drop):
        super().__init__()
...
class Global_pool(nn.Module):
    def __init__(self, num_prefix_tokens, global_pool):
        super().__init__()
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        self.pos_embed_class = Position_embed(pos_embed = self.pos_embed, cls_token = self.cls_token, no_embed_class = self.no_embed_class, pos_drop = self.pos_drop)
        ...
        self.global_pool = Global_pool(self.num_prefix_tokens, self.global_pool)
    ...
    def forward_features(self, x: torch.Tensor) -> torch.Tensor:
        #print('ff input: ', x.shape, torch.min(x), torch.max(x))
        x = self.patch_embed(x)
        #print('patch embed: ', x.shape, torch.min(x), torch.max(x))
        x = self._pos_embed(x)
        #print('position embed: ', x.shape, torch.min(x), torch.max(x))
        x = self.patch_drop(x)
        x = self.norm_pre(x)
        #print('norm pre: ', x.shape, torch.min(x), torch.max(x))
    ...
    def forward_head(self, x: torch.Tensor, pre_logits: bool = False) -> torch.Tensor:
        ...
        #print('pooling out shape: ', x.shape)
        x = self.fc_norm(x)
        x = self.head_drop(x)
        return x if pre_logits else self.head(x)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        x, kl_loss = self.forward_features(x)
