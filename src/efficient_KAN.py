...
"""
1. Memory efficiency improvement: The original implementation needs to expand all intermediate variables to perform different activation functions, while in this code the calculation is reformulated to 
  use different basis functions to activate the input, Then linearly combine them. This reformulation can significantly reduce memory costs and make computations more efficient.

2. Changes in regularization methods: The L1 regularization used in the original implementation requires non-linear operations on tensors and is incompatible with the reformulated calculations.
  Therefore, the L1 regularization in this code is changed to L1 regularization of the weights, which is more consistent with the common regularization methods in neural networks and is compatible with
  the reformulated calculation.

3. Activation function scaling option: The original implementation included learnable scaling for each activation function, but this library provides an option to disable this feature.
  Disabling scaling can make the model more efficient, but may affect results.

4. Changes in parameter initialization: In order to solve the performance problem on the MNIST data set, this code modifies the initialization method of parameters and uses kaiming initialization.
"""
...
class KANLinear(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor): # Pass the input data through each layer of the model, undergo linear transformation and activation function processing, and finally obtain the output result of the model.
        ...
        spline_output = F.linear( # Compute the output of a piecewise polynomial linear layer
    ...
    # The grid of the model is dynamically updated according to the distribution of the input data x, so that the model can better adapt to the distribution characteristics of the input data, thereby improving the expression and generalization capabilities of the model.
...
class KAN(torch.nn.Module): # 封装了一个KAN神经网络模型，可以用于对数据进行拟合和预测。
    def __init__(
        ...
        参数:
            layers_hidden (list): A list containing the number of input features for each hidden layer.
